...
public class ArtistsController {
    ...
    public ResponseEntity<UpdateArtistDTO> updateArtist(@PathVariable("id") long id, @Valid @RequestBody UpdateArtistDTO artistDTO) {
        ...
        if (artistService.updateArtist(id, artistDTO.getName(), artistDTO.getProfile())) {
        ...
        if (artists.isEmpty()) {
            ...
            return new ResponseEntity<>(
                    artists.stream().map(artist -> new ArtistDTO(
                            artist.getId(), artist.getName(), artist.getProfile(), artist.getDescription()
                    )).toList(),
                    HttpStatus.OK
            );
        }
